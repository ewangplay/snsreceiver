...
func (this *NSQProducer) Publish(topic string, body []byte) (err error) {
	retry_count := RETRY_MAX_COUNT
	for {
		err = this.GetProducer().Publish(topic, body)
		if err != nil {
			LOG_ERROR("发布消息[%v]到NSQ系统失败，失败原因：%v", string(body), err)
	...
	if err != nil {
		LOG_ERROR("发布消息[%v]到NSQ队列Topic[%v]失败，失败原因：%v", string(body), topic, err)
	} else {
		LOG_INFO("发布消息[%v]到NSQ队列Topic[%v]成功", string(body), topic)
	}
